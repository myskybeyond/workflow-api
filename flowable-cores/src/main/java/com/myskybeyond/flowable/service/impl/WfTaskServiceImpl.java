...
import com.myskybeyond.flowable.flow.CustomProcessDiagramGenerator;
import com.myskybeyond.flowable.flow.FlowableUtils;
import com.myskybeyond.flowable.mapper.WfCategoryMapper;
import com.myskybeyond.flowable.service.*;
...
@Slf4j
@Primary
public class WfTaskServiceImpl extends AbstractService implements IWfTaskService {
    ...
    public WfTaskServiceImpl(WfCategoryMapper wfCategoryMapper,
                             UserService sysUserService,
                             IWfCopyService copyService,
                             IFlowMessageService flowMessageService,
    ...
    public void complete(WfTaskBo taskBo) {
        ...
        //--增加抄送人的redis缓存
        if(StrUtil.isNotEmpty(taskBo.getCopyUserIds())){
            RedisUtils.setCacheObject(ProcessConstants.BUSINESS_FLOW_REDIS_CC_KEY+taskBo.getTaskId(), taskBo.getCopyUserIds(), Duration.ofSeconds(ProcessConstants.BUSINESS_FLOW_REDIS_CC_DURATION));
        }
        if(ObjectUtil.isNotEmpty(taskBo.getVariables())){
            //设置局部变量
            taskService.setVariablesLocal(task.getId(), taskBo.getVariables());
        }
        ...
        // 处理抄送用户
        // feat: 从流程模型获取抄送人
        constructFinalCopyUserIds(taskBo, bpmnModel, task);
        if (!copyService.makeCopy(taskBo)) {
            throw new RuntimeException("抄送任务失败");
        }
        // 填充业务数据属性
        smsTemplateVariables.fillProperties(variables, task.getProcessInstanceId(), task.getId());
        // 消息通知
        flowMessageService.send(bpmnModel, task, taskBo.getCopyUserIds(), variables);
    }
    ...
    public void delegateTask(WfTaskBo bo) {
        ...
        if (task.getState().equals(Task.SUSPENDED)) {
            throw new RuntimeException("任务已挂起不允许委派");
        }
        StringBuilder commentBuilder = new StringBuilder(LoginHelper.getUsername())
            .append("->");
    ...
    public void transferTask(WfTaskBo bo) {
        ...
        if (task.getState().equals(Task.SUSPENDED)) {
            throw new RuntimeException("任务已挂起不允许转办");
        }
        StringBuilder commentBuilder = new StringBuilder(LoginHelper.getUsername())
            .append("->");
        String nickName = sysUserService.selectNicknameById(Long.parseLong(bo.getUserId()));
...
//                        StringUtils.isBlank(flowTaskVo.getComment()) ? "取消申请" : flowTaskVo.getComment());
                ...
                // 添加变量区分人工干预结束还是自然结束
                log.warn("手动取消的流程设置取消标记至流程变量.");
                runtimeService.setVariable(bo.getProcInsId(), ProcessConstants.PROCESS_STATUS_HANDLE_TO_END, true);
                // 变更流程跳转至结束节点
                runtimeService.createChangeActivityStateBuilder()
    ...
    public void revokeProcess(WfTaskBo taskBo) {
        ...
        for (Task task : activateTaskList) {
            ...
            if (CollUtil.contains(nextUserTaskKeys, task.getTaskDefinitionKey())) {
                ...
                taskService.addComment(task.getId(), task.getProcessInstanceId(), FlowComment.REVOKE.getType(), LoginHelper.getUsername() + "撤回流程审批");
        ...
        if (CollUtil.isNotEmpty(tasks)) {
            ...
            for (Task task : tasks) {
                ...
                if (StrUtil.equals(task.getAssignee(), userIdStr)) {
                    taskService.addComment(task.getId(), processInstance.getProcessInstanceId(), FlowComment.NORMAL.getType(), LoginHelper.getUsername() + "发起流程申请");
    ...
    @Override
    public TableDataInfo<WfTaskVo> selectPageTaskList(ProcessQuery processQuery, PageQuery pageQuery) {
    ...
    @Override
    public void closeTask(WfTaskBo bo) {
        Task task = taskService.createTaskQuery().taskId(bo.getTaskId()).singleResult();
        if (Objects.isNull(task)) {
            throw new ServiceException("任务不存在");
        }
        if (task.getState().equals(Task.SUSPENDED)) {
            throw new RuntimeException("任务已挂起不允许关闭");
        }
        try {
            taskService.deleteTask(bo.getTaskId(), "管理员手动删除任务");
        } catch (FlowableException e) {
            log.error("任务: {} 删除失败,失败原因: {}", task.getId(), e.getMessage());
            throw new RuntimeException("存在运行中的流程实例不允许删除");
        }
    }

    @Override
    public void assumeTask(WfTaskBo bo) {
        Task task = taskService.createTaskQuery().taskId(bo.getTaskId()).singleResult();
        if (Objects.isNull(task)) {
            throw new ServiceException("任务不存在");
        }
        ...
        if (Objects.nonNull(processInstance)) {
            if (processInstance.isSuspended()) {
                log.error("任务挂起失败-任务id: {} 对应的流程实例: {} 已挂起,", bo.getTaskId(), bo.getProcInsId());
                throw new RuntimeException("流程实例已挂起，联系管理员！");
            }
        }
        if (task.getState().equals(Task.SUSPENDED)) {
            throw new RuntimeException("任务已挂起请勿重复操作");
        }
        taskService.addComment(bo.getTaskId(), bo.getProcInsId(), FlowComment.SUSPEND.getType(), "管理员手动挂起任务");
        taskService.suspendTask(bo.getTaskId(), TaskUtils.getUserId());
    }

    @Override
    public void activeTask(WfTaskBo bo) {
        Task task = taskService.createTaskQuery().taskId(bo.getTaskId()).singleResult();
        if (Objects.isNull(task)) {
            throw new ServiceException("任务不存在");
        }
        ...
        if (Objects.nonNull(processInstance)) {
            if (processInstance.isSuspended()) {
                log.error("任务激活失败-任务id: {} 对应的流程实例: {} 已挂起,", bo.getTaskId(), bo.getProcInsId());
                throw new RuntimeException("流程实例已挂起，联系管理员！");
            }
        }
        if (!task.getState().equals(Task.SUSPENDED)) {
            throw new RuntimeException("任务非挂起状态不用激活");
        }
        taskService.activateTask(bo.getTaskId(), TaskUtils.getUserId());
        taskService.addComment(bo.getTaskId(), bo.getProcInsId(), FlowComment.ACTIVE.getType(), "管理员手动激活任务");
    }

    @Override
    @Transactional(rollbackFor = Exception.class)
    public void startFirstTask(ProcessInstance processInstance, WfStartAndCopyBo form) {
        // 若第一个用户任务为发起人，则自动完成任务
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(processInstance.getProcessInstanceId()).list();
        if (CollUtil.isNotEmpty(tasks)) {
            ...
            for (Task task : tasks) {
                ...
                if (StrUtil.equals(task.getAssignee(), userIdStr)) {
                    ...
                    taskBo.setTaskName(task.getName());
                    taskBo.setCopyUserIds(form.getCopyUserIds());
