...
public class ModelUtils {
     ...
     * @return 用户任务节点列表
     */
    public static UserTask getFirstUserTaskEvent(BpmnModel model) {
        Process process = model.getMainProcess();
        Collection<FlowElement> flowElements = process.getFlowElements();
        Collection<UserTask> userTasks = getAllUserTaskEvent(flowElements, null);
        Iterator<UserTask> userTaskIterator = userTasks.iterator();
        if(userTaskIterator.hasNext()) {
            return userTaskIterator.next();
        }
        return null;
    }
}
