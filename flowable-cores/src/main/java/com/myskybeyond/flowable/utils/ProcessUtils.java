...
public class ProcessUtils {
    ...
    public static void buildTaskSearch(TaskQuery query, ProcessQuery process) {
        ...
        if (StringUtils.isNotBlank(process.getTaskState())) {
            // taskQuery.taskState()方法未生效不知道为什么
            // 改为 挂起->suspended() 已创建->created() 目前支持2个状态
            if(Task.SUSPENDED.equals(process.getTaskState())) {
                query.suspended();
            } else if(Task.CREATED.equals(process.getTaskState())) {
                query.active();
            } else if(Task.CLAIMED.equals(process.getTaskState())) {
                query.active().taskUnassigned();
            }else {
                log.error("任务实例查询,不支持的任务状态: {}", process.getTaskState());
            }
            query.taskState(process.getTaskState());
        }
    }
