...
public class SysDeptServiceImpl implements ISysDeptService, DeptService {
     ...
     */
    @Override
    public void checkDeptDataScopes(Long... deptIds) {
        if (ObjectUtil.isNull(deptIds)) {
            return;
        }
        if (LoginHelper.isSuperAdmin()) {
            return;
        ...
        for (Long deptId : deptIds) {
            SysDeptVo dept = baseMapper.selectDeptById(deptId);
            if (ObjectUtil.isNull(dept)) {
                throw new ServiceException("没有权限访问"+dept.getDeptName()+"数据！");
            }
        }
    }
    /**
     ...
     */
    @Override
    public void checkDeptDataScopeByUser() {
        List<Long> deptIds = selectDeptListByUserId(LoginHelper.getUserId());
        if (ObjectUtil.isNull(deptIds)) {
            return;
        }
        if (LoginHelper.isSuperAdmin()) {
    ...
    public List<Long> selectDeptListByUserId(Long userId) {
        return baseMapper.selectDeptListByUserId(userId);
    }
}
