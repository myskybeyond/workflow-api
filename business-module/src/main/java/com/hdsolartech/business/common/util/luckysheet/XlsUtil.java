...
public class XlsUtil {
     ...
     * @date 2021/4/20
     */
    public static GridRecordDataModel readExcel(Workbook workbook) {
        Iterator<Sheet> sheetIterator = workbook.sheetIterator();
        Sheet sheet = sheetIterator.next();
        int sheetIndex = 0;
        GridRecordDataModel model = strToModel("", (sheetIndex + 1) + "", 1, sheetIndex);
    ...
    private static void readSheet(Sheet sheet, GridRecordDataModel model, Workbook workbook) {
            ...
            while (cellIterator.hasNext()) {
                ...
                switch (cell.getCellType()) {
                    ...
                    case ERROR:
                        ...
                        break;
                    case FORMULA:
                        dataModel.getJSONObject("v").put("f", cell.getCellFormula());
                        try{
                            dataModel.getJSONObject("v").put("m", cell.getNumericCellValue());
                            dataModel.getJSONObject("v").put("v",  cell.getNumericCellValue());
                        }catch(Exception e){
        ...
        //celldata 数据
        model.getJson_data().put("celldata",dataList);
    }
